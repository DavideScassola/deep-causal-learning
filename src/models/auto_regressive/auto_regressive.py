...
from src.models.diffusion.diffusion_process import AutoregressiveMask
...
from src.util import (CategoricalBiasModel, edit_json, load_json,
                      logits_sample, pickle_load, pickle_store, store_json)
...
class AutoRegressive(TabularModel):
    ...
    def _generate(
        ...
        **kwargs,
        ...
        for i in range(input_dimension):
            ...
            # logit = self.demasking_predictor(x, output_targets = [i])  # model prediction: p(x | parents(x))
            # x[i] = logits_sample(logit)
    ...
    def generate_report(
        ...
        *,
        ...
        samples_tensor = self._generate(**generation_options)

        samples_df: pd.DataFrame = composed_inverse_transform(
            samples_tensor, preprocessors=self.preprocessors
        )

        store_samples(df_generated=samples_df, path=report_folder, name="samples.csv")
        ...
        with edit_json(report_folder / "stats_summary.json") as summary:
            ...
            summary["ll_train_bias_improvement"] = ll_stats["train_bias_improvement"]
