...
def fit_marginals(X: Tensor):
    return torch.log(torch.nn.functional.one_hot(X).float().mean(0))


class CategoricalBiasModel:
    def __init__(self, X: Tensor) -> None:
        self.fit(X)

    def fit(self, X: Tensor) -> None:
        self.logits = fit_marginals(X.flatten(1))

    def log_likelihood(self, X: Tensor) -> Tensor:
        return -torch.nn.functional.cross_entropy(
            self.logits.repeat(len(X), 1, 1).permute(0, 2, 1), X, reduction="none"
        )
